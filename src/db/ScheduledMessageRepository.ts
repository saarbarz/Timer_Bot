...
export class ScheduledMessageRepository {
  ...
  create(record: NewScheduledMessageRecord): ScheduledMessage {
    ...
    const created = this.findById(record.id, record.userId ?? defaultUserId);
  ...
  findById(id: string, userId?: string): ScheduledMessage | undefined {
    const row =
      userId === undefined
        ? this.db.prepare<[string], ScheduledMessageRow>("SELECT * FROM scheduled_messages WHERE id = ?").get(id)
        : this.db
            .prepare<[string, string], ScheduledMessageRow>("SELECT * FROM scheduled_messages WHERE id = ? AND user_id = ?")
            .get(id, userId);
    return row === undefined ? undefined : mapScheduledMessage(row);
  }

  list(userId?: string): ScheduledMessage[] {
    const rows =
      userId === undefined
        ? this.db
            ...
            .all()
        : this.db
            .prepare<[string], ScheduledMessageRow>(
              `
                SELECT *
                FROM scheduled_messages
                WHERE user_id = ?
  ...
  claimNextDuePending(nowUtc: string, userId?: string): ScheduledMessage | undefined {
      ...
      const due =
        userId === undefined
          ...
          : this.db
              ...
              .get(userId, claimedAtUtc, claimedAtUtc);
    ...
    return claimedId === undefined ? undefined : this.findById(claimedId, userId);
  }

  recoverStaleProcessing(staleBeforeUtc: string, updatedAtUtc: string, userId?: string): number {
    const result =
      userId === undefined
        ? this.db
            .prepare<[string, string]>(
            ...
            .prepare<[string, string, string]>(
              `
                ...
                SET
                  ...
                  next_attempt_at_utc = NULL,
                  last_error = 'stale_processing_recovered'
                WHERE user_id = ? AND status = 'processing' AND updated_at_utc <= ?
              `
            )
            .run(updatedAtUtc, userId, staleBeforeUtc);
  ...
  cancelPending(id: string, updatedAtUtc: string, userId?: string): ScheduledMessage | undefined {
    const result = this.db
      .prepare(
        `
          UPDATE scheduled_messages
      ...
      .run(...(userId === undefined ? [updatedAtUtc, id] : [updatedAtUtc, id, userId]));
  ...
  updatePendingSchedule(
    ...
    const result = this.db
      ...
      .run(
        ...(userId === undefined
          ? [scheduledAtUtc, timezone, updatedAtUtc, id]
          : [scheduledAtUtc, timezone, updatedAtUtc, id, userId])
      );

    return result.changes === 0 ? undefined : this.findById(id, userId);
  }

  updatePendingText(id: string, text: string, updatedAtUtc: string, userId?: string): ScheduledMessage | undefined {
    const result = this.db
      .prepare(
        `
          UPDATE scheduled_messages
      ...
      .run(...(userId === undefined ? [text, updatedAtUtc, id] : [text, updatedAtUtc, id, userId]));
