...
- Post-Chunk 13: the phone-number/recent-recipient field could jump while the UI refreshed. Root cause was periodic connection refresh rebuilding the native recipient datalist while the input was focused. The first fix prevented jumping but hover/focus discovery was still unreliable. Fixed by replacing the native datalist/hover behavior with a separate visible `Recent recipients from WhatsApp` dropdown under the recipient input, and using a plain ISO-style text field for `Send At` to avoid browser/OS-localized month labels.
- Post-Chunk 13: the recent-recipient dropdown could still show `No recent recipients loaded` even after Baileys provided contact data. Root cause was the recipient mapper accepting individual contacts only when the value already looked like `number@s.whatsapp.net`; Baileys can provide `phoneNumber` as a plain/E.164 phone-number string while `id` may be a non-phone LID, or provide LID contacts/chats with separate `lidPnMappings`. Fixed by normalizing contact `phoneNumber` values with spaces, `+`, parentheses, and hyphens before falling back to JID parsing, forwarding Baileys history `lidPnMappings`, and adding a sanitized `recipients: N` UI indicator.
- Post-Chunk 13: the recipient dropdown could remain empty in a live session even after UI and mapper fixes. Root cause was that `/api/recipients` depended entirely on recipient data emitted by the active in-memory Baileys session; some sessions may emit no usable contact/chat/message recipient data. Fixed by merging Baileys recipient options with local scheduled-message recipients for the current user, exposed as source `scheduled`, and adding sanitized fallback/total recipient diagnostics.
- Post-Chunk 13: enabling `BAILEYS_FULL_HISTORY_SYNC=1` could trigger repeated temporary Baileys connection closes with the desktop history profile. Fixed by automatically falling back to the normal browser profile after a temporary close, while keeping expected QR-login restart handling intact.
- Post-Chunk 13: editing a pending scheduled message opened browser prompt popups for text and send time, which made the UI feel detached from the scheduling flow. Fixed by reusing the existing schedule form for edits, prefilled with the selected message's recipient, text, date, time, and timezone, with a cancel-edit action.
